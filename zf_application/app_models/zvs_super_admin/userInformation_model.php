    ...
    public function fetchSuperAdministrators(){
        ...
        if($superAdmins == 0){
            ...
            foreach ($superAdmins as $values){
                ...
                $usersRows .= '<tr><td>'.$fullName.'</td><td>'.$userStatus.'</td><td><a href=" '.ZF_ROOT_PATH.'zvs_super_admin'.DS.'view_super_admin'.DS.$identificationCode.' " title="View '.$fullName.'" ><i class="fa fa-list"></i></a></td></tr>';
    ...
    public function fetchPlatformAdministrators(){
        ...
        if($platformAdmins == 0){
            ...
            foreach ($platformAdmins as $values){
                ...
                $fullName = $values['firstName']." ".$values['lastName'];$identificationCode = $values['identificationCode'];
                $status = $values['userStatus']; if($status == "1"){ $userStatus = "Active"; }else{ $userStatus = "Inactive"; }
                $usersRows .= '<tr><td>'.$fullName.'</td><td>'.$userStatus.'</td><td><a href=" '.ZF_ROOT_PATH.'zvs_super_admin'.DS.'view_platform_admin'.DS.$identificationCode.' "  title="View '.$fullName.'"><i class="fa fa-list"></i></a></td></tr>';
